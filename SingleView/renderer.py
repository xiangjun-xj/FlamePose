...
class Pytorch3dRasterizer(nn.Module):
    ...
    def __init__(self, image_size=224):
        ...
        raster_settings = fiting_util.dict2obj(raster_settings)
...
class Renderer(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256):
        ...
        face_uvcoords = fiting_util.face_vertices(uvcoords, uvfaces)
        ...
        colors = colors.cpu()#very strange
        face_colors = fiting_util.face_vertices(colors, faces)
    ...
    def forward(self, vertices, transformed_vertices, albedos, lights=None, light_type='point'):
        ...
        # Attributes
        face_vertices = fiting_util.face_vertices(vertices, self.faces.expand(batch_size, -1, -1))
        normals = fiting_util.vertex_normals(vertices, self.faces.expand(batch_size, -1, -1))
        face_normals = fiting_util.face_vertices(normals, self.faces.expand(batch_size, -1, -1))
        transformed_normals = fiting_util.vertex_normals(transformed_vertices, self.faces.expand(batch_size, -1, -1))
        transformed_face_normals = fiting_util.face_vertices(transformed_normals, self.faces.expand(batch_size, -1, -1))
    ...
    def render_normal(self, transformed_vertices, normals):
        ...
        # Attributes
        attributes = fiting_util.face_vertices(normals, self.faces.expand(batch_size, -1, -1))
        # rasterize
        rendering = self.rasterizer(transformed_vertices, self.faces.expand(batch_size, -1, -1), attributes)
    ...
    def world2uv(self, vertices):
        ...
        '''
        batch_size = vertices.shape[0]
        face_vertices = fiting_util.face_vertices(vertices, self.faces.expand(batch_size, -1, -1)).clone().detach()
    ...
    def save_obj(self, filename, vertices, textures):
        ...
        '''
        fiting_util.save_obj(filename, vertices, self.faces[0], textures=textures, uvcoords=self.raw_uvcoords[0],
