...
class PhotometricFitting(object):
    ...
    def _setup_renderer(self):
        mesh_file = './flame-data/head_template_mesh.obj'
    ...
    def optimize(self, images, landmarks, image_masks, savefolder=None):
        ...
        for k in range(200):
            ...
            trans_vertices = fiting_util.batch_orth_proj(vertices, cam);
            trans_vertices[..., 1:] = - trans_vertices[..., 1:]
            landmarks2d = fiting_util.batch_orth_proj(landmarks2d, cam);
            landmarks2d[..., 1:] = - landmarks2d[..., 1:]
            landmarks3d = fiting_util.batch_orth_proj(landmarks3d, cam);
            landmarks3d[..., 1:] = - landmarks3d[..., 1:]

            losses['landmark'] = fiting_util.l2_distance(landmarks2d[:, 17:, :2], gt_landmark[:, 17:, :2]) * config.w_lmks
            ...
            if k % 10 == 0:
                ...
                grids['landmarks_gt'] = torchvision.utils.make_grid(
                    fiting_util.tensor_vis_landmarks(images[visind], landmarks[visind]))
                grids['landmarks2d'] = torchvision.utils.make_grid(
                    fiting_util.tensor_vis_landmarks(images[visind], landmarks2d[visind]))
                grids['landmarks3d'] = torchvision.utils.make_grid(
                    fiting_util.tensor_vis_landmarks(images[visind], landmarks3d[visind]))
        ...
        for k in range(200, 1000):
            ...
            losses['landmark'] = fiting_util.l2_distance(landmarks2d[:, :, :2], gt_landmark[:, :, :2]) * config.w_lmks
    ...
    def run(self, imagepath, landmarkpath):
        ...
        # here we try the example with one image fitting
        images = []
        landmarks = []
        image_masks = []
        ...
        fiting_util.check_mkdir(savefolder)
        # optimize
...
if __name__ == '__main__':
    ...
    config = {
        # FLAME
        'flame_model_path': './flame-data/generic_model.pkl',  # acquire it from FLAME project page
        'flame_lmk_embedding_path': './flame-data/landmark_embedding.npy',
        'tex_space_path': './flame-data/FLAME_texture.npz',  # acquire it from FLAME project page
        'camera_params': 3,
        'shape_params': 100,
    ...
    print(type(config))
    config = fiting_util.dict2obj(config)
    print(type(config))
    fiting_util.check_mkdir(config.savefolder)

    config.batch_size = 1
    fitting = PhotometricFitting(config)
